...
from backend.settings import BROKER_PORT
...
def write_to_csv(data):
    ...
    with open(
        f"measurements/system_manager_{NUMBER_PROCESSES}p_{NUMBER_TREADS}t.csv",
        "w",
        newline="",
    ) as f:
...
def get_pids():
    backened_ppid = (
        check_output(f"lsof -n -i :{BROKER_PORT} | grep LISTEN", shell=True)
        .decode("utf-8")
        .split()[1]
    )
...
def monitor_system():
    ...
    print(formatted_avg)
    with open("momitor_system_results.txt", "+w") as file:
